...
from jetson_deploy.lcm_types.pd_tau_targets_lcmt import pd_tau_targets_lcmt
...
class LCMAgent():
    def __init__(self, cfg, se, command_profile):
        ...
        self.commands_scale = np.array(
            [self.obs_scales["lin_vel"], self.obs_scales["lin_vel"],
             self.obs_scales["ang_vel"], 1, 1, 1, 1, 1])
    ...
    def get_obs(self):
        ...
        print("A", self.is_currently_probing)
        self.commands[:, :] = self.command_profile.get_command(self.timestep * self.dt, probe=self.is_currently_probing)
        #else:
        ...
        ob = np.zeros((1, 39))
        ob[:, 0:3] = self.gravity_vector
        ob[:, 3:15] = (self.dof_pos - self.default_dof_pos) * self.obs_scales["dof_pos"]
        ob[:, 15:27] = self.dof_vel * self.obs_scales["dof_vel"]
        ob[:, 27:39] = torch.clip(self.actions, -self.cfg["normalization"]["clip_actions"],
                                  self.cfg["normalization"]["clip_actions"]).cpu().detach().numpy()

        if True:#"observe_height_command" in self.cfg["env"].keys() and self.cfg["env"]["observe_height_command"]:
            ob = np.concatenate((self.gravity_vector.reshape(1, -1),
                                 self.commands * self.commands_scale,
                                 ob[:, 3:]), axis=1)
            # print(self.commands * self.commands_scale)

        # elif self.cfg["env"]["observe_command"]:
        #     ob = np.concatenate((self.gravity_vector.reshape(1, -1),
        ...
        if self.cfg["env"]["observe_yaw"]:
            ...
            heading_error = np.clip(0.5 * wrap_to_pi(heading), -1., 1.).reshape(1, -1)
            ob = np.concatenate((ob, heading_error), axis=-1)
    ...
    def step(self, actions, hard_reset=False):
        ...
        self.clock_inputs[:, 0] = torch.sin(2 * np.pi * (self.gait_indices + phases + offsets))
        self.clock_inputs[:, 1] = torch.sin(2 * np.pi * (self.gait_indices + offsets))
        self.clock_inputs[:, 2] = torch.sin(2 * np.pi * (self.gait_indices))
        self.clock_inputs[:, 3] = torch.sin(2 * np.pi * (self.gait_indices + phases))
