...
class DeploymentRunner:
    ...
    def init_log_filename(self):
        ...
        os.makedirs(f"{self.log_root}/{datetime}")
        self.log_filename = f"{self.log_root}/{datetime}/log.pkl"
    ...
    def run(self, num_log_steps=1000000000, max_steps=100000000):
        ...
        try:
            for i in range(max_steps):
                ...
                # set command
                command = self.command_profile.get_command(self.agents[self.control_agent_name].timestep * self.agents[self.control_agent_name].dt, probe=self.is_currently_probing)
                print(self.is_currently_probing, command)
